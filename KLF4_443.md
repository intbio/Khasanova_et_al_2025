...
description: AF2-predicted structures of Klf4 complexes with nuclear proteins with observed contact to Klf4 residue 443
---
...
<div class="header text-center">
  <div class="container">
    ...
    <p>AF2-predicted structures of Klf4 complexes with nuclear proteins with observed contact to Klf4 residue 443</p>
...
const table = $('#structures-table').DataTable({
  data: dataRows.map((row, index) => {
    ...
    const gene = cols[1].trim();
    const uniprot = cols[2].trim();
    ...
    return {
      ...
      ipsae: cols[4].trim(),
      pdockq: cols[5].trim(),
      pdb: `<a href="structures/KLF4_mutations/${pdbFile}" class="download-btn" download>Download</a>`
    };
  }).filter(Boolean),
  columns: [
    { 
  ...
  try {
    if (isChecked) {
      ...
      if (!window.structureComponents[pdbFile]) {
        ...
        const pathsToTry = [
          `structures/KLF4_mutations/${pdbFile}`,
          `structures/KLF4_mutations/${pdbFile}.pdb`,
          pdbFile, 
          `${pdbFile}.pdb`
        ];
